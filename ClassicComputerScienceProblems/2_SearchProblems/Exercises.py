...
from generic_search import linear_contains, binary_contains, dfs, bfs, a_star
...
def exercise_2(num_mazes):
    ...
    for i, search in enumerate(searches):
        for _ in range(num_mazes):
            m = Maze()
            if i == 2:
                solution = search(m.start, m.goal_test, m.successors, manhattan_distance(m.goal), count=True)
            else:
