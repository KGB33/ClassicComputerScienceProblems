from generic_search import bfs, node_to_path, a_star
...
class MissionariesCannibalsState:
    ...
    def __hash__(self):
        return int(str(self.max_num) + str(self.west_missionaries)
                   + str(self.west_cannibals) + str(int(self.boat)))
    ...
    def goal_test(self):
        return self.is_legal and self.east_missionaries == self.max_num and self.east_cannibals == self.max_num
    
    def successors(self):
        heirs = []
        if self.boat:  # Boat is on the west Side
            if self.west_missionaries > 1:  # Two Missionaries on the boat
                heirs.append(MissionariesCannibalsState(
            ...
            if self.west_cannibals > 0 and self.west_missionaries > 0:  # One Of Each
                heirs.append(MissionariesCannibalsState(
                    self.west_missionaries - 1, self.west_cannibals - 1, not self.boat))
...
if __name__ == '__main__':
    ...
    solution = a_star(start, MissionariesCannibalsState.goal_test,
                      MissionariesCannibalsState.successors, MissionariesCannibalsState.min_boat_trips)
